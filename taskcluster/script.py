...
def main():
    ...
    try:
        ...
        if len(devices) != 1:
            fatal(
                "Must have exactly one connected device. {} found.".format(len(devices))
            )
    ...
    proc = subprocess.Popen(
        extra_args, env=env, stdout=subprocess.PIPE, stderr=subprocess.STDOUT
    )
    while rc == None:
    ...
    try:
        print(
            "\nnetstat -aop\n%s\n\n"
            % subprocess.check_output(["netstat", "-aop"], stderr=subprocess.STDOUT)
        )
